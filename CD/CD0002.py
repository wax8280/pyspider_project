...
class Handler(BaseHandler):
    ...
    def get_list(self, response):
        ...
        # 从js中提取左右两边的广告
        left_url = re.search(u'var coupletLeftHref="(.+?)";', response.text)
        right_url = re.search(u'var coupletRightHref="(.+?)";', response.text)

        if left_url:
            self.crawl(left_url.groups()[0], callback=self.get_target_content, validate_cert=False,
                       save={'width': '100',
                             'height': '300'}, force_update=True)
        if right_url:
            self.crawl(right_url.groups()[0], callback=self.get_target_content, validate_cert=False,
                       save={'width': '100',
                             'height': '300'}, force_update=True)
